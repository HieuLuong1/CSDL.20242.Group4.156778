...
public class Admin_CreateInventoryReportController {
    ...
    private void handleCreateReport() {
        ...
        if (reportId.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Thiếu dữ liệu", "Vui lòng nhập mã biên bản.");
            return;
        }
        if (date == null) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            String insertReportSQL = "INSERT INTO check_reports (report_id, check_date) VALUES (?, ?)";
            try (PreparedStatement psReport = conn.prepareStatement(insertReportSQL)) {
                psReport.setString(1, reportId);
                psReport.setDate(2, Date.valueOf(date));
            ...
            InventoryReport newReport = new InventoryReport(reportId, date, batchSelected, actualQuantities);
            if (reportList != null) {
